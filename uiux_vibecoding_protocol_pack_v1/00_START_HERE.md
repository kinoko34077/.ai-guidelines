...
- 機能の存在だけで完了扱いせず、ユーザーの目的達成、自然な標準操作、状態・文脈の保持、失敗からの回復を対象タスクに必要な範囲で確認する。詳細は `07_USABILITY_BASELINE.md` を参照する。
- ユーザビリティを実装・検収する場合は、`guidelines/USABILITY_POLICY.md` を上位原則として読み、具体的な判断・検収は `07_USABILITY_BASELINE.md` を参照する。UIレビューでは `08_REVIEW_CHECKLIST.md` で横断確認する。

## 2. 作業種別と参照ファイル

|作業種別|判定条件|読むファイル|出力と完了条件|
|---|---|---|---|
|A. 新規UI作成|新しい画面、機能、管理画面、エディタを作る|実装前: 01, 02, 必要時03, 04, 05, 07。実装後: 06, 08|画面目的・主要タスク・実装方針を示す。画面骨格に複数案があり方向性が未指定ならワイヤーフレーム2〜3案を提示し、合意又は妥当な既定値で実装する。実装後は06/08で検収する。|
|B. 既存UI改善|既存画面の見づらさ、使いづらさ、操作不足、密度等を改善する|01, 02, 04, 05, 必要時06, 07, 08, `templates/ui_repair_prompt.md`|問題一覧→修正方針→差分実装→変更範囲の検収。画面骨格を変えない局所修正ではワイヤーフレームを省略できる。|
|C. UIレビューのみ|コード変更なしで評価する|02, 06, 07, 08, `templates/review_output.md`|問題、根拠、優先度、修正案、未確認事項を報告して終了する。実装・ワイヤーフレームは行わない。|
